function updateViewportHeight() {
  const vh = window.innerHeight * 0.01;
  document.documentElement.style.setProperty('--vh', `${vh}px`);
}

document.addEventListener('DOMContentLoaded', updateViewportHeight);
window.addEventListener('resize', updateViewportHeight);
window.addEventListener('orientationchange', updateViewportHeight);

const burgerBtn = document.querySelector('.btn-burger');
const mobileNav = document.querySelector('.mobile-nav');

let menuScrollPosition = 0;

function lockPageScroll() {
  menuScrollPosition = window.scrollY;
  document.body.style.top = `-${menuScrollPosition}px`;
  document.body.style.setProperty('--menu-scroll-position', `${menuScrollPosition}px`);
  document.documentElement.classList.add('menu-open');
  document.body.classList.add('menu-open');
}

function unlockPageScroll() {
  document.documentElement.classList.remove('menu-open');
  document.body.classList.remove('menu-open');
  document.body.style.removeProperty('top');
  document.body.style.removeProperty('--menu-scroll-position');
  window.scrollTo(0, menuScrollPosition);
}

if (burgerBtn && mobileNav) {
  burgerBtn.addEventListener('click', () => {
    const isOpen = burgerBtn.classList.toggle('menu-open');

    burgerBtn.setAttribute('aria-expanded', isOpen);
    mobileNav.setAttribute('aria-hidden', !isOpen);

    if (isOpen) {
      lockPageScroll();
    } else {
      unlockPageScroll();
    }
  });
}

const overlay = document.querySelector('.menu-overlay');

if (overlay && burgerBtn) {
  overlay.addEventListener('click', () => {
    burgerBtn.classList.remove('menu-open');
    burgerBtn.setAttribute('aria-expanded', false);

    unlockPageScroll();
    mobileNav.setAttribute('aria-hidden', true);
  });
}

const range = document.getElementById('loan-range');
const loanAmountEl = document.getElementById('loan-amount');
const returnNewEl = document.getElementById('return-new');
const returnOldEl = document.getElementById('return-old');

const INTEREST_RATE = 1.24;

function formatRub(value) {
  return value.toLocaleString('ru-RU');
}

function updateRangeFill(el) {
  const min = Number(el.min) || 0;
  const max = Number(el.max) || 100;
  const val = Number(el.value) || 0;

  const percent = ((val - min) * 100) / (max - min);
  el.style.setProperty('--p', `${percent}%`);
}

function updateCalculator(value) {
  const amount = Number(value);
  const oldAmount = Math.round(amount * INTEREST_RATE);

  loanAmountEl.textContent = formatRub(amount);
  returnNewEl.textContent = formatRub(amount);
  returnOldEl.textContent = formatRub(oldAmount);
  updateRangeFill(range);
}

if (range && loanAmountEl && returnNewEl && returnOldEl) {
  range.addEventListener('input', (e) => {
    updateCalculator(e.target.value);
  });

  updateCalculator(range.value);
}

// ===== Bottom sheets (универсально) =====
const sheetOverlay = document.querySelector('[data-sheet-overlay]');
const sheetCloseBtns = document.querySelectorAll('[data-sheet-close]');

function openSheetById(sheetId, triggerBtn) {
  const sheet = document.getElementById(sheetId);
  if (!sheet || !sheetOverlay) return;

  // 1) Закрываем все другие bottom-sheet
  document.querySelectorAll('.bottom-sheet').forEach(s => {
    s.setAttribute('aria-hidden', 'true');
  });

  // 2) Сбрасываем aria-expanded у всех триггеров
  document.querySelectorAll('[data-sheet-target]').forEach(btn => {
    btn.setAttribute('aria-expanded', 'false');
  });

  // 3) Открываем нужный
  document.body.classList.add('sheet-open');
  sheet.setAttribute('aria-hidden', 'false');
  sheetOverlay.setAttribute('aria-hidden', 'false');

  if (triggerBtn) triggerBtn.setAttribute('aria-expanded', 'true');
}

function closeAllSheets() {
  document.body.classList.remove('sheet-open');

  document.querySelectorAll('.bottom-sheet[aria-hidden="false"]').forEach(s => {
    s.setAttribute('aria-hidden', 'true');
  });

  if (sheetOverlay) sheetOverlay.setAttribute('aria-hidden', 'true');

  // сбрасываем aria-expanded у всех триггеров
  document.querySelectorAll('[aria-controls]').forEach(btn => {
    btn.setAttribute('aria-expanded', 'false');
  });
}

// Открытие по data-sheet-target (надежнее)
document.querySelectorAll('[data-sheet-target]').forEach(btn => {
  btn.addEventListener('click', () => {
    const id = btn.getAttribute('data-sheet-target');
    openSheetById(id, btn);
  });
});

if (sheetOverlay) sheetOverlay.addEventListener('click', closeAllSheets);
sheetCloseBtns.forEach(b => b.addEventListener('click', closeAllSheets));

// ===== Features slider dots (optional) =====
const featuresTrack = document.querySelector('.features--slider');
const dots = document.querySelectorAll('.features-dots .dot');

if (featuresTrack && dots.length) {
  const items = Array.from(featuresTrack.querySelectorAll('.feature'));

  function setActiveDot(index) {
    dots.forEach((d, i) => d.classList.toggle('is-active', i === index));
  }

  function nearestIndex() {
    const left = featuresTrack.scrollLeft;
    let best = 0;
    let bestDist = Infinity;

    items.forEach((item, i) => {
      const dist = Math.abs(item.offsetLeft - left);
      if (dist < bestDist) { bestDist = dist; best = i; }
    });

    return best;
  }

  let raf = null;
  featuresTrack.addEventListener('scroll', () => {
    if (raf) cancelAnimationFrame(raf);
    raf = requestAnimationFrame(() => setActiveDot(nearestIndex()));
  });

  dots.forEach((dot, i) => {
    dot.addEventListener('click', () => {
      items[i].scrollIntoView({ behavior: 'smooth', inline: 'start' });
    });
  });
}

// ===== Deposit calculator (invest) =====
(function initDepositCalc(){
  const termRange = document.getElementById('dep-term-range');
  const termOut   = document.getElementById('dep-term');

  const amountRange = document.getElementById('dep-amount-range');
  const amountInput = document.getElementById('dep-amount-input');

  const topup = document.getElementById('dep-topup');

  const rateEl    = document.getElementById('dep-rate');
  const rateOldEl = document.getElementById('dep-rate-old');

  const yieldEl = document.getElementById('dep-yield');
  const totalEl = document.getElementById('dep-total');
  const deltaEl = document.getElementById('dep-delta');

  if (!termRange || !termOut || !amountRange || !amountInput || !rateEl || !yieldEl || !totalEl) return;

  // ===== helper: плавное обновление текста =====
  function setTextAnimated(el, text){
    if (!el) return;
    if (el.textContent === text) return;

    el.classList.add('js-anim','is-updating');
    requestAnimationFrame(() => {
      el.textContent = text;
      el.classList.remove('is-updating');
    });
  }

  const rateByMonths = {
    1: 22, 2: 22, 3: 22, 4: 28, 5: 28, 6: 30,
    7: 30, 8: 30, 9: 30, 10: 30, 11: 30, 12: 30
  };

  const TOPUP_PENALTY = 3;

  function clamp(n, min, max){
    return Math.min(max, Math.max(min, n));
  }

  function formatInt(n){
    const v = Math.round(Number(n) || 0);
    return v.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ');
  }

  function parseAmount(value){
  return Number(String(value).replace(/[^\d]/g, '')) || 0;
}

function normalizeAmount(value){
  const min = Number(amountRange.min);
  const max = Number(amountRange.max);
  const step = Number(amountRange.step) || 1;
  const clamped = clamp(parseAmount(value), min, max);

  return Math.round(clamped / step) * step;
}

  function updateRangeFill(el){
    const min = Number(el.min) || 0;
    const max = Number(el.max) || 100;
    const val = Number(el.value) || 0;
    const percent = ((val - min) * 100) / (max - min);
    el.style.setProperty('--p', `${percent}%`);
  }

  function baseRate(months){
    return rateByMonths[months] ?? rateByMonths[12] ?? 30;
  }

  function calc(syncInput = true){
  const months = clamp(Number(termRange.value), 1, 12);
  setTextAnimated(termOut, String(months));

  const amount = normalizeAmount(amountInput.value);
  amountRange.value = amount;

  if (syncInput) {
    amountInput.value = `${formatInt(amount)} ₽`;
  }

  const r0 = baseRate(months);
  const isTopup = !!(topup && topup.checked);
  const r = isTopup ? Math.max(0, r0 - TOPUP_PENALTY) : r0;

  setTextAnimated(rateEl, r.toFixed(2));

  if (rateOldEl){
    rateOldEl.textContent = isTopup ? `${r0.toFixed(2)}%` : '';
  }

  const profit = amount * (r / 100) * (months / 12);
  const total = amount + profit;

  setTextAnimated(yieldEl, formatInt(profit));
  setTextAnimated(totalEl, formatInt(total));

  if (deltaEl){
    const deltaText = profit > 0 ? `↑ ${formatInt(profit)} ₽` : '';
    setTextAnimated(deltaEl, deltaText);
  }

  updateRangeFill(termRange);
  updateRangeFill(amountRange);
}

  termRange.addEventListener('input', calc);

  amountRange.addEventListener('input', () => {
  amountInput.value = `${formatInt(amountRange.value)} ₽`;
  calc();
});

  amountInput.addEventListener('input', () => {
  const rawAmount = parseAmount(amountInput.value);
  const minAmount = Number(amountRange.min);

  if (!amountInput.value || rawAmount < minAmount) {
  amountRange.value = minAmount;
  updateRangeFill(amountRange);

  const amount = minAmount;
  const months = clamp(Number(termRange.value), 1, 12);
  const r0 = baseRate(months);
  const isTopup = !!(topup && topup.checked);
  const r = isTopup ? Math.max(0, r0 - TOPUP_PENALTY) : r0;
  const profit = amount * (r / 100) * (months / 12);
  const total = amount + profit;

  setTextAnimated(yieldEl, formatInt(profit));
  setTextAnimated(totalEl, formatInt(total));

  if (deltaEl) {
    setTextAnimated(deltaEl, profit > 0 ? `↑ ${formatInt(profit)} ₽` : '');
  }

  return;
}

  amountRange.value = normalizeAmount(rawAmount);
  calc(false);
});

amountInput.addEventListener('blur', () => {
  calc(true);
});

  if (topup) topup.addEventListener('change', calc);

  calc();
})();
