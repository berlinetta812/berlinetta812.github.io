...
(function initDepositCalc(){
  ...
  const amountUI    = document.getElementById('dep-amount-ui');
  ...
  if (!termRange || !termOut || !amountRange || !amountUI || !rateEl || !yieldEl || !totalEl) return;

  // Пример таблицы ставок по сроку (можешь заменить цифры позже)
  // "до 30%" достигается на 12 мес.
  const rateByMonths = {
    1: 22, 2: 22, 3: 22, 4: 28, 5: 28, 6: 30,
    7: 30, 8: 30, 9: 30, 10: 30, 11: 30, 12: 30
  };

  // -3% если пополняемый (автоматически, без текста)
  ...
  function calc(){
    ...
    termOut.textContent = String(months);

    // amount
    const amount = clamp(Number(amountRange.value), Number(amountRange.min), Number(amountRange.max));
    amountUI.textContent = formatInt(amount);
    ...
    rateEl.textContent = r.toFixed(2);

    if (rateOldEl){
      rateOldEl.textContent = isTopup ? `${r0.toFixed(2)}%` : '';
    }

    // profit (простая модель, MVP): простые проценты
    const profit = amount * (r / 100) * (months / 12);
    const total = amount + profit;

    yieldEl.textContent = formatInt(profit);
    totalEl.textContent = formatInt(total);

    if (deltaEl){
      deltaEl.textContent = profit > 0 ? `↑ ${formatInt(profit)} ₽` : '';
    }

    // fill for ranges (под твоё CSS с --p)
