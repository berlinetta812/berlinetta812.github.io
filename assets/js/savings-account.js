...
function getShortAccountNumber(id) {
  const normalized = String(id || '').replace(/\D/g, '');
  return normalized.slice(-10) || String(id || '—').slice(-12);
}
...
function renderInterestSchedule(schedule) {
  ...
  function updateList() {
    ...
    filtered.forEach((item) => {
      list.append(
        createActivityItem(
          `Плановое начисление ${formatDayMonth(item.date)}`,
          item.date,
          item.amount,
          'neutral'
        )
    ...
    filtered.forEach((operation) => {
      ...
      list.append(
        createActivityItem(
          ...
          operation.createdAt,
          operation.amount,
          isWithdrawal ? 'negative' : 'positive'
        )
      );
    });
  }
...
function renderAccount() {
  ...
  if (
    ...
    if (account.status === 'payout_pending') {
      ...
      document.querySelector('[data-maturity-message]').textContent =
        'Проценты больше не начисляются. Ожидайте возврата всей суммы.';
  ...
  document.querySelector('[data-account-number]').textContent =
    getShortAccountNumber(account.id);
...
document.querySelector('[data-fund-account]')?.addEventListener('click', () => {
  ...
  const shouldFund = window.confirm(
    `Имитировать пополнение на ${formatMoney(fundingAmount)}?`
  );
...
document.querySelector('[data-request-maturity-payout]')?.addEventListener('click', () => {
  ...
  const shouldRequest = window.confirm(
    `Подать заявку на получение ${formatMoney(payoutAmount)}?`
  );
